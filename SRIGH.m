...
% Cparams.c : location of the sink
% 
...
print_to_files(Cparams.c, x, sparams.O, fij, fiB);
end
...
%   x: binary vector of optimal node deployment
%   O: deployment sites
%   f: flow matrix
%   ff: connection with the sink
%

function print_to_files(c, x, O, f, ff)
pos = fopen('~/pos','w');
tot = size(O, 1);
for i=1:tot
    if x(i) == 1
